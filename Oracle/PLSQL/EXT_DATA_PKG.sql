...
/* PRC 06/09.05: Removed SPecial Literal Processing */
 IS

-- Global Variable Declarations

   G$Special_Literal   Char := ' ';
...
FUNCTION F_CT_DATA_TX_EXIST
 ...
 ,P_KEY3        IN VARCHAR2
 ,P_LINE_TEXT   IN VARCHAR2
 ,P_VERSION     IN VARCHAR2) -- PRC 12/15/2005
 RETURN VARCHAR2;
...
--step 0 Set Global Processing Variables
   T_STEP_NAME := '0';

   BEGIN
      SELECT SPECIAL_LITERAL INTO G$SPECIAL_LITERAL
...
BEGIN
  ...
  BEGIN
    ...
    -- PRC 2/28/08: Removed "TRUNCATE" command in favor of "DELETE", so data related
    --              to the specific protocol can be targetted for deletion.
    DELETE FROM CT_EXT_TEMP where PROTOCOL = P_STUDY;
    ...
    ---------------------
    --prc      FOR T_REC IN (SELECT * FROM CT_EXT_TEMP) LOOP
    -- PRC 2/28/08: Removed simple loop in favor of study specific loop, so data related
    --              to the specific protocol can be targetted for query.

    FOR T_REC IN (SELECT * FROM CT_EXT_TEMP WHERE PROTOCOL = P_STUDY) LOOP
...
PROCEDURE P_INS_CRS_AE
 ...
 IS
  ...
  T_RESLV_DT           VARCHAR2(8);
  T_OUTCOME            VARCHAR2(3);
  T_LINE_TEXT          VARCHAR2(2400);
  T_LAST_COURSE_START_DT   VARCHAR2(30) := NULL;
  T_Max_COURSE_START_DT    VARCHAR2(30) := NULL;
  I                    NUMBER := 0;


  T_CT_CUR1      CT_CURTYPE;
  T_CT_CUR2      CT_CURTYPE;
  T_CT_CUR3      CT_CURTYPE;
  T_GC_MAX_COURSE_START_DT          LONG;
  T_GC_LAST_COURSE_START_DT         LONG;
  T_GC_COURSE_DT_NUM                LONG;
BEGIN
  Log_Util.LogMessage('PICA - Starting, P_COUNT="'||To_Char(P_Count)||'".');
  Log_Util.LogMessage('PICA - Onset Date="'||SUBSTR(P_CT_TAB(P_COUNT).LINE_TEXT,148,8)||'"."');
  Log_Util.LogMessage('PICA - Toxicity="'||SUBSTR(P_CT_TAB(P_COUNT).LINE_TEXT,115,33)||'"."');

  T_reslv_DT := SUBSTR(P_CT_TAB(P_COUNT).LINE_TEXT, 166, 8); -- Hold for Later
  T_OutCome  := SUBSTR(P_CT_TAB(P_COUNT).LINE_TEXT, 183, 1); -- Hold for Later
  Log_Util.LogMessage('PICA - T_reslv_DT="'||T_reslv_DT||'"."');
  Log_Util.LogMessage('PICA -  T_OutCome="'||T_OutCome||'"."');

  If Rtrim(T_Reslv_DT) = '-6' Then
     T_Max_Course_Start_DT := '99999999';
  Else
     SELECT TEXT INTO T_GC_MAX_COURSE_START_DT
       FROM CT_EXT_CRS_CTL
      WHERE CRS_NAME = 'GC_MAX_COURSE_START_DT'
        AND FILE_ID = 'TX';

     T_GC_MAX_COURSE_START_DT := REPLACE(T_GC_MAX_COURSE_START_DT, 'PP_EXT_OWNER', P_EXT_OWNER);

     BEGIN
        Log_Util.LogMessage('PICA - P_CT_TAB('||to_char(P_COUNT)||').PROTOCOL="'||
                                    P_CT_TAB(P_COUNT).PROTOCOL||'"');
        Log_Util.LogMessage('PICA - P_CT_TAB('||to_char(P_COUNT)||').PT="'||
                                    P_CT_TAB(P_COUNT).PT||'"');
        Log_Util.LogMessage('PICA - T_Reslv_DT="'||T_Reslv_DT||'"');
        OPEN T_CT_CUR3 FOR T_GC_MAX_COURSE_START_DT
                           USING P_CT_TAB(P_COUNT).PROTOCOL, P_CT_TAB(P_COUNT).PT, T_Reslv_DT;
        FETCH T_CT_CUR3 INTO T_Max_COURSE_START_DT;
        CLOSE T_CT_CUR3;
     EXCEPTION
        WHEN OTHERS THEN
           IF T_CT_CUR3%ISOPEN THEN
              CLOSE T_CT_CUR3;
           END IF;
           RAISE;
     END;
  End If;
  Log_Util.LogMessage('PICA - T_Max_COURSE_START_DT="'||T_Max_COURSE_START_DT||'"."');

/*****************/
  Log_Util.LogMessage('PICA - Getting Last Course Start_Date');
  ...
  BEGIN
    ...
    Log_Util.LogMessage('PICA - P_CT_TAB('||to_char(P_COUNT)||').PT="'||
                                    P_CT_TAB(P_COUNT).PT||'"');
    Log_Util.LogMessage('PICA - P_CT_TAB('||to_char(P_COUNT)||').KEY3="'||
                                    P_CT_TAB(P_COUNT).KEY3||'"');
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      RAISE;
  END;
  Log_Util.LogMessage('PICA - T_LAST_COURSE_START_DT="'||T_LAST_COURSE_START_DT||'"');
  ...
  Log_Util.LogMessage('PICA - Starting INSERT Loop');
  ...
  Log_Util.LogMessage('PICA - P_CT_TAB('||to_char(P_COUNT)||').PT="'||
                                    P_CT_TAB(P_COUNT).PT||'"');
  Log_Util.LogMessage('PICA - P_CT_TAB('||to_char(P_COUNT)||').KEY5="'||
                                    P_CT_TAB(P_COUNT).KEY5||'"');
  Log_Util.LogMessage('PICA - T_LAST_COURSE_START_DT="'||T_LAST_COURSE_START_DT||'"');
  Log_Util.LogMessage('PICA - P_CT_TAB('||to_char(P_COUNT)||').KEY4="'||
                                    P_CT_TAB(P_COUNT).KEY4||'"');
  ...
  LOOP
    ...
    Log_Util.LogMessage('PICA - T_COURSE_NUM="'||T_COURSE_NUM||'"');
    Log_Util.LogMessage('PICA - T_START_DT="'||T_START_DT||'"');
    Log_Util.LogMessage('PICA - (1)I="'||to_Char(I)||'"');
    Log_Util.LogMessage('PICA - (1)P_COUNT="'||TO_CHAR(P_COUNT)||'"');
    EXIT WHEN T_CT_CUR2%NOTFOUND;
       ...
       ELSE
         I := I + 1;
       END IF;
    Log_Util.LogMessage('PICA - (2)I="'||to_Char(I)||'"');
    Log_Util.LogMessage('PICA - (2)P_COUNT="'||TO_CHAR(P_COUNT)||'"');
...
--           P_CT_TAB(I).KEY4        := P_CT_TAB(P_COUNT).KEY4;
           ...
           If P_Version = 'T312' Then -- ONLY for VERSION T312
              If T_START_DT = T_Max_COURSE_START_DT Then --Include Resolved Date, Regardless of value
                 P_CT_TAB(I).LINE_TEXT   := SUBSTR(P_CT_TAB(P_COUNT).LINE_TEXT, 1, 91)||T_COURSE_NUM||
                                            ...
                                            SUBSTR(P_CT_TAB(P_COUNT).LINE_TEXT, 115,51)||T_Reslv_dt||
                                            SUBSTR(P_CT_TAB(P_COUNT).LINE_TEXT, 174,9)
                                            ||T_OutCome;
              Else -- Put a -6 (ONGOING) in the resolved date for LESSOR Courses.
                 P_CT_TAB(I).LINE_TEXT   := SUBSTR(P_CT_TAB(P_COUNT).LINE_TEXT, 1, 91)||T_COURSE_NUM||
                                            SUBSTR(P_CT_TAB(P_COUNT).LINE_TEXT, 95, 12)||T_START_DT||
                                            SUBSTR(P_CT_TAB(P_COUNT).LINE_TEXT, 115,51)||'-6      '||
                                            SUBSTR(P_CT_TAB(P_COUNT).LINE_TEXT, 174,9);
                 If T_OutCome = '*' Then
                    P_CT_TAB(I).LINE_TEXT := P_CT_TAB(I).LINE_TEXT || '*';
                 Else
                    P_CT_TAB(I).LINE_TEXT := P_CT_TAB(I).LINE_TEXT || ' ';
                 End If;
              End If;
           ...
           End If;
           Log_Util.LogMessage('PICA -       P_CT_TAB(I).LINE_TEXT="'||P_CT_TAB(I).LINE_TEXT||'"');
           Log_Util.LogMessage('PICA - P_CT_TAB(P_COUNT).LINE_TEXT="'||P_CT_TAB(P_COUNT).LINE_TEXT||'"');
  ...
  Log_Util.LogMessage('PICA - Finished.');
...
BEGIN
-----------------------
   If log_util.loggetname is null Then
      Log_Util.LogSetName('AE_EXT_' || P_STUDY || '_' || to_char(sysdate, 'YYMMDD-HH24'),'AE_EXTRACT');
   End If;

T_STEP_NAME := '2.1';          ----DEL_FLAG, step 1(normal records)
Log_Util.LogMessage('AE - Step' || T_STEP_NAME);
BEGIN
  ...
  BEGIN
    ...
    LOOP
      ...
      P_INS_CRS_AE(T_CT_TAB, I, P_EXT_OWNER, P_DEBUG_MODE, P_VERSION);
...
-----------------------
-----------------------
-- Start Processing Normal Adverse Event Records : prc 12/21/05
T_STEP_NAME := '3';          ----normal records
Log_Util.LogMessage('AE - Step' || T_STEP_NAME);
BEGIN
--   Log_Util.LogSetName('AEDEBUG_' || to_char(sysdate, 'YYYYMMDD-HH24MI'),'EXTRACT');
   Log_Util.LogMessage('AE - Starting "Normal Records".');

  -- Get the Cursor Text for Study/ReportTo/Version : prc 12/21/05
  SELECT TEXT, EXT_SOURCE INTO T_CRS, T_EXT_SOURCE
    FROM CT_EXT_CRS_CTL
   WHERE OC_STUDY = P_STUDY
     AND REPORT_TO = P_REPORT_TO
  ...
  -- Replace Owner Hold Variable with REAL value  : prc 12/21/05
  T_CRS := REPLACE(T_CRS, 'PP_EXT_OWNER', P_EXT_OWNER);

  -- Update Step Name, used with Exception Handler to locate errors : prc 12/21/05
  ...
  BEGIN
    ...
    Log_Util.LogMessage('AE - '||T_STEP_NAME);

    -- Write to the Extract Log, what is processing : prc 12/21/05
    EXT_UTIL_PKG.P_CT_EXT_LOGS
            (P_STUDY, P_FILE_ID, T_PROC_NAME, P_CURRENT_EXT_DATE, T_EXT_STATUS, T_START_DATE, SYSDATE
            ,NULL, NULL, NULL, NULL, P_DEBUG_MODE);
    ...
    -- Open the AE Cursor, passing in Variables needed by Cursor
    -- These Should ALWAYS be a placed holder in the cursor for each variable
    -- : prc 12/21/05
    OPEN C_TX FOR T_CRS USING P_FILE_ID, P_VERSION, P_CURRENT_EXT_DATE
                             ,P_FILE_ID, P_VERSION, P_CURRENT_EXT_DATE
                             ,P_STUDY, P_FILE_ID, P_STUDY, P_FILE_ID, P_STUDY;

    -- Set Array Start to 0, Empty Record Array : prc 12/21/05
    I := 0;  T_CT_TAB := T_EMPTY_TAB;

    T_STEP_NAME := '3.2';          ----normal records
    Log_Util.LogMessage('AE - '||T_STEP_NAME);
    -- Begin Loop That will Process Every Single Row Returned by AE Cursor : prc 12/21/05
    LOOP
      I := I + 1;
      T_STEP_NAME := '3.3.'||I;          ----normal records
      Log_Util.LogMessage('AE - '||T_STEP_NAME);
      -- Get the Record and place it into Record Array : prc 12/21/05
      FETCH C_TX INTO T_CT_TAB(I);
      EXIT WHEN C_TX%NOTFOUND;

      Log_Util.LogMessage('AE - Calling P_INS_CRS_AE');
      -- Call Routine that Creates Records for EACH course that the AE Spans : prc 12/21/05
      P_INS_CRS_AE(T_CT_TAB, I, P_EXT_OWNER, P_DEBUG_MODE, P_VERSION);
  ...
  -----------
    ...
    Log_Util.LogMessage('AE - Starting Loop.');

    I := 0;
    -- If there is something in the processing Record Array : prc 12/21/05
    IF NVL(T_CT_TAB.LAST, 0) <> 0 THEN
      -- For every Record in the Record Array : prc 12/21/05
      FOR I_TX IN T_CT_TAB.FIRST..T_CT_TAB.LAST LOOP
        Log_Util.LogMessage('AE - I_TX="'||To_Char(I_TX)||'".');
        Log_Util.LogMessage('AE - I="'||To_Char(I)||'".');
       -- Check to see if the record has already be reported : prc 12/21/05
       IF F_CT_DATA_TX_EXIST(T_CT_TAB(I_TX).FILE_ID, T_CT_TAB(I_TX).PROTOCOL,
                              ...
                              T_CT_TAB(I_TX).LINE_TEXT,
                              T_CT_TAB(I_TX).VERSION) = 'N'
        THEN
           -- Only copy NOT REPORTED records to NEW Record Array : prc 12/21/05
           Log_Util.LogMessage('AE - Inside IF');
           Log_Util.LogMessage('AE - I_TX="'||To_Char(I_TX)||'".');
           I := I + 1;
           ...
           Log_Util.LogMessage('AE - T_CT_TAB_TX(I).LINE_TEXT="'||T_CT_TAB_TX(I).LINE_TEXT||'".');
        END IF;
      END LOOP;
    END IF;
  -----------
  -----------
    IF I > 0 THEN
      -- Take records from Record Array, Write them to Candidate Table : prc 12/21/05
...
T_STEP_NAME := '2';          ----normal records
    ...
    T_SQL_STR :=
     ...
     '      AND A.DEL_FLAG <> ''D'' ';  -- PRC 01/17/2006: Ingnore deletes, because enrollment may be deleted
      ...
      EXIT WHEN GC_CT_DATA_TX_EXIST%NOTFOUND;
        ...
        ELSE
           IF P_VERSION = 'T312' Then
              IF '  '||SUBSTR(P_LINE_TEXT, 56) =
                 '  '||SUBSTR(T_REC.LINE_TEXT, 56) Then
                 T_FLAG := 'Y';
                 EXIT;
              End If;
           Else
